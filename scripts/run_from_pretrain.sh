...
if [[ ${#SEED_ARR[@]} -lt ${#WEIGHT_FILES[@]} ]]; then
  echo "Warning: fewer seeds (${#SEED_ARR[@]}) than weight files (${#WEIGHT_FILES[@]}). Some weights will be reused seeds." >&2
fi
...
for WEIGHT_PATH in "${WEIGHT_FILES[@]}"; do
  ...
  # This ends up in args.exp_name and thus in run_name and W&B run name
...
echo "All runs for ${ALGO} on ${ENV_ID} finished (launched sequentially)."
